...
class Tracker:
    ...
    def __init__(self, metric, max_iou_dist=0.9, max_age=30, max_unmatched_preds=7, n_init=3, _lambda=0, ema_alpha=0.9, mc_lambda=0.995):
        self.metric = metric
        self.max_iou_dist = max_iou_dist   # 0.543
        self.max_age = max_age             # 40
        self.n_init = n_init               # 3
        self._lambda = _lambda             # 0
        self.ema_alpha = ema_alpha         # 0.896
        self.mc_lambda = mc_lambda         # 0.995
        self.max_unmatched_preds = max_unmatched_preds  # 0
    ...
    def _match(self, detections):
        ...
        matches_a, unmatched_tracks_a, unmatched_detections = \
            linear_assignment.matching_cascade(
                ...
                self.tracks, detections, confirmed_tracks)  # 根据激活态轨迹保存的外观信息与预测框对应的外观信息作匹配
        ...
        matches_b, unmatched_tracks_b, unmatched_detections = \
            linear_assignment.min_cost_matching(
                ...
                detections, iou_track_candidates, unmatched_detections)  # 集合上一轮中的未确定态轨迹索引和第一次未匹配的轨迹索引获得所有未匹配的轨迹，之后这些轨迹与上轮未匹配的预测框计算Iou
                                                                         # 根据轨迹与预测框的iou，再次进行一次匈牙利匹配，最终获得匹配结果matches_b、未匹配轨迹unmatched_tracks_b和未匹配的预测框
